...
// http://www.esp8266.com/viewtopic.php?f=32&t=4634 makes note of the 
// command "server.client().print()" which is 160x faster than server.sendContent()
// someone report a transfer speed for server.send of 10 KB in 0,15 - 0,2 seconds
// using "String str = "";str += F("<!DOCTYPE html>");str += F("<html>");..."
...
const char gui[] PROGMEM = 
  ...
  "</head>\n"
  "<body>\n<h1 style='font-family:Lucida Console;font-size:32px'>VIC-II for ESP8266/C64</h1>\n"
...
//    "}\n"
    ...
    "  printLog('refreshVICRAM');\n"
    "  xmlhttp.onreadystatechange=function(){\n"
    "    if (xmlhttp.readyState==4 && xmlhttp.status==200){\n"
    "      var aB=xmlhttp.response;\n"
    "      if (aB){\n"
    "        for (var i=0;i<1001;i++){\n"
    "          VICRAM[i]=aB.charCodeAt(i);\n"
    "        }\n"
    "      }\n"
    "      refreshDisplay();\n"
    ...
    "  if (keyp.length>0) printLog('post: '+url);\n"
    "  keyp='';\n"
...
//    "  printLog('refreshDisplay');\n"
    ...
    "  ctx.fillStyle='#4140E4';\n"
    "  ctx.fillRect(32,32,320,200);\n"
    "  ctx.textBaseline='top';\n"
    "  ctx.fillStyle='#A4A4FE';\n"
    "  var petscii;\n"
    "  for (var col=0;col<40;col++){\n"
    "    for (var row=0;row<40;row++){\n"
    "      petscii=VICRAM[row*40+col];\n"
    "      if (petscii<32) petscii = petscii + 64;\n"
...
void VICIIRefresh(){
  ...
  //send video ram
  char msg[1024];//one byte per value
  for (int i=1024;i<2023;i++){
    uint8_t petscii = RAM[i];
    if (petscii<32) petscii = petscii + 64;
    msg[i-1024]=char(petscii);
    int c=(i-1024)%40;
  };
...
int fase=0;
void loop () {
  if (fase!=3) {Serial.print ("Start boot fase ");Serial.println(fase);}
  if (fase==0) {initWifi();}
  if (fase==1) {initServer();}
  if (fase==2) {reset6502();}
  if (fase!=3) {Serial.print ("..finished boot fase ");Serial.println(fase);}
  if (fase!=3) {fase++;return;}
