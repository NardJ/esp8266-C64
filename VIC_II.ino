...
void send_uint8ws(uint8_t num, const uint8_t *data, const size_t data_length) {

  webSocket.sendBIN(num, data, data_length);
...
uint8_t VICRAM_OLD[1024];

void VICIIRefreshWS(uint8_t num) {
  wdt_disable();
  ...
  //send_uint8(VICRAM,1024);
  if ( memcmp( (const void *)VICRAM, (const void *)VICRAM_OLD, sizeof(VICRAM)) == 0)
  {
    //    wdt_enable(WDTO_0MS);
    wdt_enable(5000);
    return;
  ...
  send_uint8ws(num, VICRAM, 1024);
  memcpy( ( void *)VICRAM_OLD, ( void *)VICRAM, sizeof(VICRAM));

  //  webSocket.sendTXT(num, VICRAM, 1024);
  //dumpScreenRam();
  wdt_enable(5000);
  //  wdt_enable(WDTO_0MS);
}

void GetChar(uint8_t * payload, size_t length) {
  wdt_disable();
